...
# print(final_df['se_diff'].max()) # trying to figure out how to scale colors for map

### PLOTLY

with urlopen("https://raw.githubusercontent.com/plotly/datasets/master/geojson-counties-fips.json") as response:
    county_ids = json.load(response)

with urlopen("https://raw.githubusercontent.com/PublicaMundi/MappingAPI/master/data/geojson/us-states.json") as response:
    state_ids = json.load(response)
...
fig = px.choropleth_mapbox(
    ...
    range_color=[0,30], # max of se_diff is 29.4%
    opacity=0.6,
    mapbox_style="carto-positron",
    zoom=3,
    center={"lat": 37.8, "lon": -96},
    labels={"se_diff" : "Percent"},
    title="Percentage difference in males and females with majors in SE",
