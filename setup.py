...
os.chdir('C:/Users/sophi/OneDrive/Documents/US Majors by County')
...
counties = {}
for year in years:
    df = pd.read_csv(f'ACSST1Y{year}.S1502-Data.csv')
...
# bad_counties = sorted(list(bad_counties)) 
# just sorting to check for duplicates which is unnecessary
...
# might be more interesting and informative to show the difference between female and male stem majors, and is a good way
# to get a sense of how the major percentage stacks up against the state average (eg, if a percent is small generally or
# actually small for that state relative to male percent)
...
# in map, make color harsher/worse when difference is larger and paler when difference is less
# should put info about total percentage of stem majors in hover

majors = {}
for year in years:
    majors[year] = {
    'se_diff' : new_data[year]['S1502_C04_008E'].astype("float") - new_data[year]['S1502_C06_008E'].astype("float"),
    'ser_diff' : new_data[year]['S1502_C04_009E'].astype("float") - new_data[year]['S1502_C06_009E'].astype("float"),
    'bus_diff' : new_data[year]['S1502_C04_010E'].astype("float") - new_data[year]['S1502_C06_010E'].astype("float"),
    'edu_diff' : new_data[year]['S1502_C04_011E'].astype("float") - new_data[year]['S1502_C06_011E'].astype("float"),
    'hum_diff' : new_data[year]['S1502_C04_012E'].astype("float") - new_data[year]['S1502_C06_012E'].astype("float")
    }
# print(majors[2015]['hum_diff']) # works!
...
final_df = pd.concat(frames, ignore_index=True) # ignore_index=True very important for avoiding index like 0 1 0 1 instead of 0 1 2 3

print(final_df['se_diff'].max())
